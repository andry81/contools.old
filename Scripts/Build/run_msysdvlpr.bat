...
call "%%CONTOOLS_OLD_ROOT%%\setvarsfromfile.bat" "%%CONFIG_PATH%%\msysdvlpr.vars"
...
call "%%CONTOOLS_ROOT%%\msys\msysver.bat" msys "%%MSYS_PATH%%"
if %ERRORLEVEL% NEQ 0 (
...
"%MSYS_PATH%\bin\bash" "%CONTOOLS_BASH_ROOT%/unmountdir.sh" "/usr/local"
if %ERRORLEVEL% EQU 0 echo /usr/local

"%MSYS_PATH%\bin\bash" "%CONTOOLS_BASH_ROOT%/unmountdir.sh" "/usr"
...
"%MSYS_PATH%\bin\bash" "%CONTOOLS_BASH_ROOT%/mountdir.sh" "%TMP%" "/tmp"
if %ERRORLEVEL% NEQ 0 (
...
"%MSYS_PATH%\bin\bash" "%CONTOOLS_BASH_ROOT%/mountdir.sh" "%MINGW_PATH%" "/mingw"
if %ERRORLEVEL% NEQ 0 (
  call :cecho %%~nx0: {0F}MINGW_PATH{#}: "{0F}%%MINGW_PATH%%{#}">&2
...
(
  ...
  set "CONTOOLS_OLD_ROOT=%CONTOOLS_OLD_ROOT%"
  set "MSYS_PATH=%MSYS_PATH%"
  set "MINGW_PATH=%MINGW_PATH%"
  set "CONFIG_PATH=%CONFIG_PATH%"

  call :RESET_ENV_AND_CALL vars_win2k.lst "%%CONTOOLS_OLD_ROOT_NATIVE%%" "%%CONFIG_PATH%%" "%%MSYS_PATH%%" "%%MINGW_PATH%%"
)
exit /b
...
(
  ...
  call :RESET_ENV_AND_CALL vars_winxp.lst "%%CONTOOLS_OLD_ROOT_NATIVE%%" "%%CONFIG_PATH%%" "%%MSYS_PATH%%" "%%MINGW_PATH%%"
)
exit /b
...
(
  ...
  call :RESET_ENV_AND_CALL vars_vista.lst "%%CONTOOLS_OLD_ROOT_NATIVE%%" "%%CONFIG_PATH%%" "%%MSYS_PATH%%" "%%MINGW_PATH%%"
)
exit /b
...
(
  ...
  call :RESET_ENV_AND_CALL vars_win7.lst "%%CONTOOLS_OLD_ROOT_NATIVE%%" "%%CONFIG_PATH%%" "%%MSYS_PATH%%" "%%MINGW_PATH%%"
)
exit /b
...
(
  ...
  call :RESET_ENV_AND_CALL vars_win8.lst "%%CONTOOLS_OLD_ROOT_NATIVE%%" "%%CONFIG_PATH%%" "%%MSYS_PATH%%" "%%MINGW_PATH%%"
)
exit /b
...
call :cecho %%~nx0: {0B}info{#}: "{0F}CONTOOLS_OLD_ROOT=%%CONTOOLS_OLD_ROOT%%{#}"
echo.
