...
call "%%CONTOOLS_OLD_ROOT%%\setvarsfromfile.bat" "%%CONFIG_PATH%%\msys.vars"
...
"%MSYS_PATH%\bin\bash" "%CONTOOLS_OLD_ROOT%/unmountdir.sh" "/usr/local"
if %ERRORLEVEL% EQU 0 echo /usr/local

"%MSYS_PATH%\bin\bash" "%CONTOOLS_OLD_ROOT%/unmountdir.sh" "/usr"
...
"%MSYS_PATH%\bin\bash" "%CONTOOLS_OLD_ROOT%/mountdir.sh" "%TMP%" "/tmp"
if %ERRORLEVEL% NEQ 0 (
...
"%MSYS_PATH%\bin\bash" "%CONTOOLS_OLD_ROOT%/mountdir.sh" "%MINGW_PATH%" "/mingw"
if %ERRORLEVEL% NEQ 0 (
  call :cecho %%~nx0: {0F}MINGW_PATH{#}: "{0F}%%MINGW_PATH%%{#}">&2
