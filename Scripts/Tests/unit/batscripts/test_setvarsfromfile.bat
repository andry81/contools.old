...
pushd "%TEST_DATA_BASE_DIR%/test_setvarsfromfile" && (
  call "%%CONTOOLS_OLD_ROOT%%/setvarsfromfile.bat" %%~1 "%%~2"
)
popd
...
"%CONTOOLS_UTILS_BIN_ROOT%/contools/printf.exe" "=${VARVALUE_ACTUAL}=${VARVALUE_REFERENCE}="

"%CONTOOLS_UTILS_BIN_ROOT%/contools/envvarcmp.exe" VARVALUE_ACTUAL VARVALUE_REFERENCE "" ^
