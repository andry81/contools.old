...
call "%%CONTOOLS_OLD_ROOT%%/nsis/gen_nsis_defines.bat" 1251 "%%BUILD_CONFIG_ROOT%%\nsis_defines.lst" > "%PROJECT_STAGE_BUILD_ROOT.GEN_DIR%/nsis_defines.nsi"
if %ERRORLEVEL% NEQ 0 (
...
if %F_ENABLE_APP_DIR_INSTALL_FROM_ARCHIVE%0 NEQ 0 (
  call "%%CONTOOLS_OLD_ROOT%%/gen_dir_files_list_from_archive.bat" 1251 ^
    "%%STAGE_IN.PROJECT_STAGE_POSTBUILD_ROOT.BIN_DIR%%/*.7z" "*.*" > "%PROJECT_STAGE_BUILD_ROOT.GEN_DIR%/product_install_file_list.lst"
) else (
  call "%%CONTOOLS_OLD_ROOT%%/gen_dir_files_list.bat" 1251 ^
...
if %F_ENABLE_APP_DIR_INSTALL_FROM_ARCHIVE%0 NEQ 0 (
  call "%%CONTOOLS_OLD_ROOT%%/nsis/gen_install_files_section_from_archive.bat" 1251 $EXEDIR "" "" "*.7z" "%%PROJECT_STAGE_BUILD_ROOT.GEN_DIR%%" ^
    0 "%%STAGE_IN.PROJECT_STAGE_POSTBUILD_ROOT.BIN_DIR%%" > "%PROJECT_STAGE_BUILD_ROOT.GEN_DIR%/install.nsi"
) else (
  call "%%CONTOOLS_OLD_ROOT%%/nsis/gen_install_files_section.bat" 1251 APP_ROOT "" "" ^
...
if %F_ENABLE_APP_DIR_INSTALL_FROM_ARCHIVE%0 NEQ 0 (
  call "%%CONTOOLS_OLD_ROOT%%/nsis/gen_uninstall_files_section_from_archive.bat" 1251 $INSTDIR "" "*.7z" ^
    "%%STAGE_IN.PROJECT_STAGE_POSTBUILD_ROOT.BIN_DIR%%" > "%PROJECT_STAGE_BUILD_ROOT.GEN_DIR%/uninstall.nsi"
) else (
  call "%%CONTOOLS_OLD_ROOT%%/nsis/gen_uninstall_files_section.bat" 1251 $INSTDIR "" "" ^
    "%%STAGE_IN.PROJECT_STAGE_POSTBUILD_ROOT.BIN_DIR%%" > "%PROJECT_STAGE_BUILD_ROOT.GEN_DIR%/uninstall.nsi"
)
if %ERRORLEVEL% NEQ 0 (
  echo.%~nx0: error: Build stopped because of previous errors.>&2
...
if %F_ENABLE_APP_INTEGRATION_DIR_INSTALL_FROM_ARCHIVE%0 NEQ 0 (
  ...
  call "%%CONTOOLS_OLD_ROOT%%/nsis/gen_install_files_section.bat" 1251 APP_INTEGRATION_ROOT "" "" ^
...
call "%%CONTOOLS_OLD_ROOT%%/nsis/gen_files_pp_command_from_list.bat" 1251 SCRIPT ".bat|.cmd" ^
...
call "%%CONTOOLS_OLD_ROOT%%/nsis/gen_files_pp_command_from_list.bat" 1251 PROC ".exe" ^
...
if %LAST_ERROR% NEQ 0 (
  call "%%CONTOOLS_OLD_ROOT%%/nsis/find_errors_nsis_log.bat" "%%MAKENSIS_LOG_FILE_NAME%%"
  echo.
  echo.%~nx0: error: Build stopped because of previous errors.>&2
  exit /b 70
)
